...
export default function Activitybar({ isSidebarOpen, setIsSidebarOpen }: ActivitybarProps) {
    ...
    return (
        <div className="w-16 h-full bg-tokyo-sidebar flex flex-col justify-between items-center py-4 border-r border-[#16161e]">
            <div className="flex flex-col items-center w-full gap-4">
                <button
                    onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                    className={`w-full py-3 flex justify-center relative group transition-colors duration-200 ${
                        isSidebarOpen ? 'text-tokyo-accent border-l-2 border-tokyo-accent' : 'text-gray-500 hover:text-tokyo-text'    
                    }`}
                    title="Explorer"
                >
                ...
                <button 
                    className="w-full py-3 flex justify-center text-gray-500 hover:text-tokyo-text transition-colors duration-200"
                    title="Search"
                >
                    <Search size={24} />
                </button>
